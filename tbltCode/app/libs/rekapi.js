...
(function(global) {
  ...
  var rekapiCore = function(root, _, Tweenable) {
     ...
     * @chainable
     */
    function Rekapi(opt_context, opt_document) {
      this.context = opt_context || {};
      this._document = opt_document || document;
      this._actors = {};
  ...
  rekapiModules.push(function(context) {
    ...
    function invalidatePropertyCache(actor) {
      ...
      _.each(actor._keyframeProperties, function(keyframeProperty) {
        ...
        timelinePropertyCache[millisecond][keyframeProperty.name] = keyframeProperty;
      });
    ...
    Actor.prototype.hasKeyframeAt = function(millisecond, opt_trackName) {
      ...
      var track;
      for (track in tracks) {
        if (tracks.hasOwnProperty(track) && findPropertyAtMillisecondInTrack(this, track, millisecond)) {
          return true;
        }
      }
    ...
    Actor.prototype.getKeyframeProperty = function(property, millisecond) {
      ...
      if (propertyTrack) {
        return _.findWhere(propertyTrack, {
          millisecond: millisecond
        });
      }
    };
  ...
  rekapiModules.push(function(context) {
    ...
    function injectStyle(rekapi, css) {
      var style = rekapi._document.createElement('style');
      var id = 'rekapi-' + styleID++;
      style.id = id;
      style.innerHTML = css;

      rekapi._document.head.appendChild(style);
    ...
    function forceStyleReset(rekapi) {
      var dummyDiv = rekapi._document.createElement('div');
    ...
    DOMRenderer.prototype.stop = function(opt_goToEnd) {
      if (this.isPlaying()) {
        ...
        this.rekapi._document.head.removeChild(this._styleElement);
        this._styleElement = null;
        ...
        if (opt_goToEnd) {
          ...
          updateTime = (now() - this._playTimestamp) % this.rekapi.getAnimationLength();
        }
    ...
    function applyVendorBoilerplates(toKeyframes, animName, opt_vendors) {
      ...
      _.each(opt_vendors, function(vendor) {
        var renderedChunk = printf(KEYFRAME_TEMPLATE, [VENDOR_PREFIXES[vendor], animName, toKeyframes]);
    ...
    function generateAnimationIterationProperty(rekapi, prefix, opt_iterations) {
      ...
      if (opt_iterations) {
        ...
        iterationCount = rekapi._timesToIterate === -1 ? 'infinite' : rekapi._timesToIterate;
      }
    ...
    function generateOptimizedKeyframeSegment(
      ...
      accumulator.push(printf('  %s% {%s:%s;%sanimation-timing-function: %s;}', [adjustedFromPercent, generalName, property.value, VENDOR_TOKEN, timingFnChunk]));
      accumulator.push(printf('  %s% {%s:%s;}', [adjustedToPercent, generalName, property.nextProperty.value]));
    ...
    function simulateLeadingWait(actor, track, actorStart) {
      ...
      if (typeof firstProp !== 'undefined' && firstProp.millisecond !== actorStart) {
    ...
    function simulateTrailingWait(actor, track, actorStart, actorEnd) {
      ...
      if (typeof lastProp !== 'undefined' && lastProp.millisecond !== actorEnd) {
